...
export default function EquipmentRequestsPage() {
  ...
  const requests = bookings.filter(b => b.type === 'EQUIPMENT');
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <div>
            ...
            <h1 style={{ fontSize: '2.3rem', color: 'var(--color-indigo)', margin: 0 }}>
              🔔 Incoming Tractor / Tool Rental Requests
            </h1>
          </div>
          <Link href="/equipment/listing">
            <button className="btn btn-leaf" style={{ padding: '8px 16px', fontSize: '0.95rem' }}>My Machinery</button>
          </Link>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          {requests.length === 0 ? (
            <div className="card" style={{ textAlign: 'center', padding: '4rem 2rem' }}>
              ...
              <h2 style={{ color: 'var(--color-soil)' }}>No rental requests currently.</h2>
              <p style={{ color: 'var(--color-soil)', opacity: 0.8, marginTop: '0.5rem' }}>
                When farmers rent your machinery from `/agri/equipment`, requests will arrive here immediately.
              </p>
            </div>
          ) : (
            requests.map(req => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  <div>
                    <h2 style={{ margin: 0, color: 'var(--color-indigo)', fontSize: '1.3rem' }}>{req.customerName}</h2>
                ...
                <p style={{ margin: '0 0 0.5rem 0', color: 'var(--color-soil)', fontWeight: 'bold' }}>{req.title}</p>
                {req.notes && <p style={{ margin: '0 0 0.5rem 0', color: 'var(--color-soil)', fontStyle: 'italic', fontSize: '0.9rem' }}>&ldquo;{req.notes}&rdquo;</p>}
                <p style={{ margin: '0 0 1.25rem 0', color: 'var(--color-soil)', opacity: 0.8, fontSize: '0.85rem' }}>
                  📅 Requested: {req.date} {req.timeSlot ? `• Duration: ${req.timeSlot}` : ''}
                </p>
                ...
                {req.status === 'ACCEPTED' && (
                  <div style={{ display: 'flex', gap: '1rem' }}>
                    <button 
                      ...
                      style={{ flex: 1 }}
                    >
                      ✓ Mark Rental Completed (Collected ₹{req.amount})
                    </button>
                    <button 
                      onClick={() => alert(`Calling farmer ${req.customerName}...`)} 
